(() => {
  ...
  const SAVE_KEY = 'seiiki-frontier-save-v7-orbits';
  const AS = 'assets/img/';
  const VERSION = 7;
  ...
  function mapFrame(){
    ...
    return {left:8, right:76, top: state?.tutorial?.active ? 196 : 92, bottom: Math.max(98, bottomH + 10)};
  }
  ...
  function canvasPointerDown(e){ drag.active=true; drag.moved=false; drag.startX=e.clientX; drag.startY=e.clientY; drag.lastX=e.clientX; drag.lastY=e.clientY; canvas.setPointerCapture?.(e.pointerId); }
  function canvasPointerMove(e){ if(!drag.active) return; const dx=e.clientX-drag.lastX, dy=e.clientY-drag.lastY; if(Math.hypot(e.clientX-drag.startX,e.clientY-drag.startY)>6) drag.moved=true; const a=screenToWorld(0,0), b=screenToWorld(dx,dy); camera.x=clamp(camera.x-(b.x-a.x),0,1); camera.y=clamp(camera.y-(b.y-a.y),0,1); drag.lastX=e.clientX; drag.lastY=e.clientY; draw(); }
  function canvasPointerUp(e){ if(!drag.active) return; drag.active=false; if(!drag.moved){ const s=nearestSystemAt(e.clientX,e.clientY); if(s){ state.selectedId=s.id; advanceTutorialOnSelect(s); save(); render(); } } }
  ...
  function bind(){
    ...
    $('helpBtn').onclick=showHelp; $('turnChip').onclick=showApHelp; $('saveBtn').onclick=()=>{save(); toast('保存しました。次回は「続きから」で再開できます。');}; $('endTurnBtn').onclick=endTurn; $('modalClose').onclick=()=>{modalPage=null; $('infoModal').close();}; $('coachSkipBtn').onclick=()=>{state.tutorial.active=false; state.tutorial.step=99; render();}; $('sheetToggle').onclick=()=>{state.sheetCollapsed=!state.sheetCollapsed; renderPanel(); save();};
    document.querySelectorAll('#sideMenu [data-page]').forEach(b=>b.onclick=()=>openMobilePage(b.dataset.page));
    canvas=$('starCanvas'); canvas.addEventListener('pointerdown',canvasPointerDown); canvas.addEventListener('pointermove',canvasPointerMove); canvas.addEventListener('pointerup',canvasPointerUp); canvas.addEventListener('pointercancel',()=>drag.active=false); addEventListener('resize',resizeCanvas);
  }

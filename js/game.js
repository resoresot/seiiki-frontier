(() => {
  ...
  const SIEGE_STAR_CAPTURE = 30;
  const HOME_INVASION_CHARGE = 2;
  const MAX_DEVELOPMENT = 6;
  // Tech victory final project ("銀河同期プロジェクト"): final tech only makes you eligible;
  // you then push a 100-point project so tech victory lands a few turns after eligibility.
  const TECH_PROJECT_TARGET = 100;
  const TECH_PROJECT_STEP = 22;
  const TECH_PROJECT_CPU_STEP = 16;
  const TECH_PROJECT_COST = {research:40, energy:18, crystal:9, credits:20};
  ...
  const TECHS = [
    ...
    {id:'galacticMandate', name:'銀河統治網', icon:'victory', cost:150, tier:4, req:['capitalProtocol'], final:true, victoryType:'tech', text:'技術勝利。星域全体を統治する最終技術です。銀河同期プロジェクトの起点になります。'},
    ...
    {id:'singularityCore', name:'特異点中枢', icon:'victory', cost:160, tier:4, doctrine:'tech', req:['aiCommand'], final:true, victoryType:'tech', text:'技術勝利。圧倒的な演算力で銀河を制御します。銀河同期プロジェクトの起点になります。'},
    ...
    {id:'conquestFleet', name:'制圧艦隊', icon:'victory', cost:142, tier:4, doctrine:'war', req:['siegeDoctrine'], final:true, victoryType:'military', text:'軍事ルート補助。母星攻略を後押しする最終艦隊技術です。'},
    ...
    {id:'galacticMarket', name:'銀河市場', icon:'victory', cost:142, tier:4, doctrine:'trade', req:['energyMarket'], final:true, victoryType:'economy', text:'経済・内政ルート補助。経済圏で銀河を包む最終交易技術です。'},
    ...
    {id:'lifeweb', name:'生命圏ネットワーク', icon:'victory', cost:140, tier:4, doctrine:'eco', req:['livingShield'], final:true, victoryType:'eco', text:'生態・内政ルート補助。巨大生態圏を完成させる最終技術です。'},
    ...
    {id:'voidCrown', name:'虚空の王冠', icon:'victory', cost:145, tier:4, doctrine:'mystic', req:['phaseShield'], final:true, victoryType:'mystic', text:'特殊技術ルート補助。常識外の星域支配を可能にする最終技術です。'},
  ...
  TECHS.push(
    ...
    {id:'postScarcity', name:'希少性後経済', icon:'credits', cost:230, tier:5, branch:'economy', req:['megaFoundries','tariffUnion','galacticMarket'], final:true, victoryType:'economy', text:'経済・内政ルート補助。資源制約を超える銀河経済圏を完成します。'},
    {id:'livingGalaxy', name:'生きた銀河', icon:'food', cost:228, tier:5, doctrine:'eco', branch:'bio', req:['symbioticHabitats','gaiaEngines','lifeweb'], final:true, victoryType:'eco', text:'生態・内政ルート補助。植民地すべてを巨大生命圏として接続します。'},
    {id:'starMind', name:'恒星知性', icon:'technology', cost:232, tier:5, doctrine:'tech', branch:'science', req:['syntheticAdvisors','omegaArchive','singularityCore'], final:true, victoryType:'tech', text:'技術勝利。恒星規模の演算知性を完成します。銀河同期プロジェクトの起点になります。'},
    {id:'totalConquest', name:'総征服令', icon:'victory', cost:226, tier:5, doctrine:'war', branch:'military', req:['imperialNavy','warEconomy','conquestFleet'], final:true, victoryType:'military', text:'軍事ルート補助。全星系を制圧するための最終軍事体制。'},
    {id:'tradeHegemony', name:'交易覇権', icon:'victory', cost:224, tier:5, doctrine:'trade', branch:'economy', req:['tariffUnion','monopolyCharter','galacticMarket'], final:true, victoryType:'economy', text:'経済・内政ルート補助。全勢力の補給線を自国経済圏に組み込みます。'},
    {id:'oracleNexus', name:'神託接続体', icon:'victory', cost:226, tier:5, doctrine:'mystic', branch:'void', req:['ghostFleets','voidPilgrimage','voidCrown'], final:true, victoryType:'mystic', text:'特殊技術ルート補助。虚空経路から銀河秩序を上書きします。'}
  );
  ...
  function createGame(seed, doctrine){
    ...
    return {version:VERSION, seed, turn:1, ap:3, maxAp:3, selectedId:2, systems, factions, logs:[], tutorial:{active:true,step:0}, replay:[], victory:null, defeat:null, sieges:{}, techProject:{}, sheetCollapsed:false, milestones:{}};
  }
  ...
  function buildCluster(systems, factions, c){
    ...
    const addBody = (body, kind, name, dx, dy, opts={}) => {
      ...
      systems.push({
        ...
        origin:opts.origin || (opts.owner?'native':null), occupiedTurn:0
      });
  ...
  function normalizeState(){
    state.version = VERSION; state.logs ||= []; state.replay ||= []; state.tutorial ||= {active:false,step:99}; state.maxAp ||= 3; state.ap ??= state.maxAp; state.techProject ||= {}; state.factions.forEach(f=>{ f.charge ||= 0; f.maxCharge ||= 1; f.ships ||= {scout:1,frigate:0,destroyer:0,carrier:0}; f.upgrades ||= {weapons:0,shields:0,engines:0}; f.techs ||= []; });
    // Old saves predate origin tracking: treat already-owned bodies as self-built so they are not penalized.
    state.systems.forEach(s=>{ if(s.origin===undefined) s.origin = s.owner ? 'native' : null; if(s.occupiedTurn===undefined) s.occupiedTurn=0; });
  }
  ...
  function completeTutorial(){
    ...
    showModal(`<h2>チュートリアル完了</h2><p>ここからは自由に戦略を選べます。</p><ul><li><b>恒星ハーネス</b>を研究すると、恒星をチャージ拠点として使えます。</li><li><b>ワープ航法</b>を研究し、恒星チャージを貯めると、別帝国の星系へ侵攻できます。</li><li>侵攻では、保有艦船・武器/シールド/推進・相手の防衛施設タイプが結果を左右します。</li><li>敵母星を落とすと、その帝国は敗退し、残った保有星も接収できます。</li><li>勝利条件は、支配60%、技術勝利（研究系の最終技術＋銀河同期プロジェクト完了）、CPU母星制圧の3つです。不明点は右の「説明」を見てください。</li></ul><p>では、銀河の主導権を取りにいきましょう。</p>`, true);
  }

  function render(){ if(!state) return; renderTop(); renderCoach(); renderPanel(); renderTutorialHighlights(); renderReplay(); draw(); if(!checkDefeat()) checkVictory(); save(); }
  ...
  // Count T4+ techs that belong to one of the given research branches (final techs are branch 'final' → excluded).
  function t4InBranches(fid, branches){ const f=faction(fid); return TECHS.filter(t=>(t.tier||1)>=4 && f.techs.includes(t.id) && branches.includes(techBranch(t))).length; }
  function researchBases(fid){ return owned(fid).filter(s=>s.level>=2 && ['home','ice','relic','crystal'].includes(s.body)); }
  function researchBaseCount(fid){ return researchBases(fid).length; }
  function researchBaseLevelSum(fid){ return researchBases(fid).reduce((a,s)=>a+s.level,0); }
  // Only finals explicitly flagged victoryType:'tech' can drive the tech victory.
  function techVictoryFinals(fid){ const f=faction(fid); return f.techs.filter(id=>{ const t=TECHS.find(x=>x.id===id); return t?.final && t.victoryType==='tech'; }); }
  function hasTechVictoryFinal(fid){ return techVictoryFinals(fid).length>0; }
  function techProjectProgress(fid){ return Math.min(TECH_PROJECT_TARGET, Math.floor((state.techProject||{})[fid]||0)); }
  // Prerequisites that must be met before the synchronization project can advance (and before a tech final is researchable).
  function techVictoryGate(fid){
    const inc=totalIncome(fid), reasons=[];
    ...
    if(researchBaseLevelSum(fid)<10) reasons.push(`研究拠点Lv合計 ${researchBaseLevelSum(fid)} / 10`);
    ...
    if(t4InBranches(fid,['science'])<1) reasons.push(`研究系T4 ${t4InBranches(fid,['science'])} / 1`);
    if(t4InBranches(fid,['warp'])<1) reasons.push(`恒星系T4 ${t4InBranches(fid,['warp'])} / 1`);
    if(t4InBranches(fid,['industry','defense','economy','void'])<1) reasons.push(`工業/防衛/交易/虚空T4 ${t4InBranches(fid,['industry','defense','economy','void'])} / 1`);
    return reasons;
  }
  function canTechProject(fid='P'){ return hasTechVictoryFinal(fid) && techVictoryGate(fid).length===0 && techProjectProgress(fid)<TECH_PROJECT_TARGET; }
  function effectiveControl(fid){
    ...
    // Self-colonized / self-developed worlds count for more than freshly seized ones; recent captures count for the least.
    const originWeight=(s)=>{
      if(s.origin==='occupied'){ const age=state.turn-(s.occupiedTurn||0); return age<5 ? 0.55 : 0.8; }
      if(s.origin==='colonized' || s.origin==='native' || s.origin===undefined) return 1.12;
      return 1;
    };
    let mine=all.filter(s=>s.owner===fid).reduce((a,s)=>a+scoreOf(s)*originWeight(s),0);
    ...
    const f=faction(fid);
    const adminBonus=Math.min(0.06, adminTechCount(fid)*0.012);
    let domTech=0;
    if(f.techs.includes('sectorCapital')) domTech+=0.03;
    if(f.techs.includes('autonomousColonies')) domTech+=0.03;
    if(f.techs.includes('galaxyLogistics')) domTech+=0.04;
    const rate=Math.min(1.2, (mine*drag)/total*(1+adminBonus)+domTech);
    return {score:mine*drag,total,rate,red};
  }
  ...
  function techVictoryReasons(fid){
    const reasons=techVictoryGate(fid);
    if(!hasTechVictoryFinal(fid)) reasons.unshift('技術勝利用の最終技術が未完成');
    if(techProjectProgress(fid)<TECH_PROJECT_TARGET) reasons.push(`銀河同期 ${techProjectProgress(fid)} / ${TECH_PROJECT_TARGET}`);
    return reasons;
  }
  // Researching a tech-victory final requires the full tech-victory breadth gate.
  // Doctrine finals (military/economy/eco/mystic) are now ordinary strong techs gated only by their tech-tree prerequisites.
  function finalResearchReasons(t, fid='P'){
    if(!t?.final || t.victoryType!=='tech') return [];
    return techVictoryGate(fid);
  }
  function siegeKeyFor(s){ return s?.homeOf || s?.owner || ''; }
  // Per-besiege progress step. War techs and capital-succession doctrine accelerate the siege so the military route keeps tempo.
  function siegeStepBonus(fid, s){
    const f=faction(fid);
    return SIEGE_STEP_BASE
      + (f.techs.includes('siegeDoctrine')?10:0)
      + (f.techs.includes('marines')?6:0)
      + (f.techs.includes('capitalProtocol')?8:0)
      + (f.techs.includes('orbitalDrop')?5:0)
      + (f.doctrine==='war'?4:0)
      + Math.min(10,ownedInCluster(fid,s.cluster).length*2);
  }
...
function handleAction(action,arg){ if(!tutorialAllows(action,arg)){ toast('チュートリアル中は、表示された手順だけ実行できます。'); return; } const before=state.tutorial.step; const ok = ({explore,colonize,develop,fortify,chargeStar,besiege,invade,buildShip,upgrade,research,techProject}[action] || (()=>false))(arg); if(ok) afterTutorialAction(action,arg); render(); }
  ...
  function besiege(){
    ...
    const bonus=siegeStepBonus('P',s);
  ...
  function techProject(){
    const fid='P';
    if(!hasTechVictoryFinal(fid)) return toast('銀河同期プロジェクトには、技術勝利用の最終技術が必要です。');
    if(techVictoryGate(fid).length) return toast(`同期の前提が未達：${techVictoryGate(fid).slice(0,2).join(' / ')}`);
    if(techProjectProgress(fid)>=TECH_PROJECT_TARGET) return toast('銀河同期は完了済みです。');
    if(!canPay(TECH_PROJECT_COST)) return toast(`資源不足：${costText(TECH_PROJECT_COST)}`);
    if(!consumeAp()) return false;
    pay(TECH_PROJECT_COST);
    if(!state.techProject) state.techProject={};
    state.techProject[fid]=Math.min(TECH_PROJECT_TARGET,(state.techProject[fid]||0)+TECH_PROJECT_STEP);
    const prog=state.techProject[fid];
    addLog(`銀河同期プロジェクトを進行。同期${prog}/${TECH_PROJECT_TARGET}。`, prog>=TECH_PROJECT_TARGET?'good':'');
    return true;
  }
  ...
  function executeInvasion(attackerId, targetId, opts={}){
    ...
    if(success){
      ...
      if(s.kind==='star' && s.starOf && s.starOf!==attacker.id){
        const key=s.starOf; state.sieges[key]=Math.min(100,(state.sieges[key]||0)+SIEGE_STAR_CAPTURE);
        if(attacker.id==='P') addLog(`${oldName}の恒星を制圧。母星包囲が大きく前進（包囲${state.sieges[key]}/100）。`, 'good');
      }
  ...
  function eliminateEmpire(enemy, newOwner='P'){
    ...
    state.systems.forEach(s=>{ if(s.owner===enemy.id){ s.owner=newOwner; s.origin='occupied'; s.occupiedTurn=state.turn; if(newOwner==='P') s.explored=true; } });
  }
  ...
  function cpuTurn(){
    ...
    state.factions.filter(f=>f.id!=='P'&&!f.eliminated).forEach(f=>{
      ...
      // Eligible CPUs push the synchronization project automatically, so their tech victory also lands a few turns after eligibility.
      if(hasTechVictoryFinal(f.id) && techVictoryGate(f.id).length===0 && techProjectProgress(f.id)<TECH_PROJECT_TARGET){
        if(!state.techProject) state.techProject={};
        state.techProject[f.id]=Math.min(TECH_PROJECT_TARGET,(state.techProject[f.id]||0)+TECH_PROJECT_CPU_STEP);
      }
    });
    return events;
  }
  function chooseCpuAction(f){
    ...
    if(state.turn>8 && f.techs.includes('warpDrive') && (f.charge||0)>0 && pTargets.length && fleetStats(f).power>Math.max(28, fleetStats(player()).power*.40)){
      ...
      if(state.turn>=VICTORY_TURN && playerHome && homeInvasionReasons(playerHome,f.id).length===0){ target=playerHome; }
        ...
        const key=siegeKeyFor(playerHome); state.sieges[key]=Math.min(100,(state.sieges[key]||0)+siegeStepBonus(f.id,playerHome));
    ...
    if(neutral && f.resources.materials>=22 && f.resources.influence>=10){ f.resources.materials-=22; f.resources.influence-=10; neutral.owner=f.id; neutral.origin='colonized'; neutral.occupiedTurn=0; neutral.level=0; neutral.pop=1; neutral.explored=true; return {fid:f.id,sid:neutral.id,text:`${f.name}が${neutral.name}へ植民。`,type:'colonize'}; }
  ...
  function techProjectPanelHtml(){
    const prog=techProjectProgress('P');
    // Only show once a tech-victory final exists or the project is already under way.
    if(!hasTechVictoryFinal('P') && prog<=0) return '';
    const gate=techVictoryGate('P'), eligible=gate.length===0, done=prog>=TECH_PROJECT_TARGET;
    const can=eligible && !done && canPay(TECH_PROJECT_COST) && state.ap>0;
    const label = done ? '同期完了' : can ? `同期を進める（${costText(TECH_PROJECT_COST)} / AP1）` : eligible ? '資源/AP不足' : '前提未達';
    const note = done ? '<p class="good-note">銀河同期が完了しました。技術勝利が成立します。</p>'
      : eligible ? `<p>各回 +${TECH_PROJECT_STEP}。${Math.ceil((TECH_PROJECT_TARGET-prog)/TECH_PROJECT_STEP)}回で完了します。</p>`
      : `<p class="missing-note">前提：${gate.slice(0,3).join(' / ')}</p>`;
    return `<section class="panel-section tech-project"><h2>銀河同期プロジェクト</h2><p>同期 <b>${prog}/${TECH_PROJECT_TARGET}</b>。最終技術の力を銀河全体へ同期させ、進捗100で技術勝利します。</p>${note}<div class="tech-detail-actions"><button class="primary-btn${can?'':' disabled'}" data-action="techProject" type="button">${label}</button></div></section>`;
  }
  function techHtml(){
    const p=player();
  ...
  function victoryProgressHtml(fid='P'){
    const eff=effectiveControl(fid), avg=averageDevelopment(fid), inc=totalIncome(fid), admin=adminTechCount(fid), bases=researchBaseCount(fid), baseLv=researchBaseLevelSum(fid);
    const sciT4=t4InBranches(fid,['science']), warpT4=t4InBranches(fid,['warp']), miscT4=t4InBranches(fid,['industry','defense','economy','void']), proj=techProjectProgress(fid);
    ...
    return `<section class="panel-section victory-panel"><h2>勝利ルート進捗</h2><p>全ルートはTurn${VICTORY_TURN}以降に決着します。Turn50までは準備・橋頭堡・統治・研究基盤を作る時間です。</p>
      <div class="route-grid"><div><b>内政</b><span>${Math.round(eff.rate*100)}%</span><small>平均Lv${avg.toFixed(1)} / 行政${admin}/4 / ${dom.length?dom[0]:'達成可'}</small></div><div><b>技術</b><span>同期${proj}/${TECH_PROJECT_TARGET}</span><small>拠点${bases}/3・Lv計${baseLv}/10 / T4 研${sciT4} 恒${warpT4} 他${miscT4} / 電${signFmt(inc.energy||0)} 結${signFmt(inc.crystal||0)} / ${tech.length?tech[0]:'達成可'}</small></div><div><b>軍事</b><span>${enemies.filter(e=>e.eliminated).length}/4母星</span><small>母星攻略は技術・橋頭堡・包囲・チャージが必要</small></div></div>
  ...
  function codexHtml(){
    ...
    return `<section class="panel-section"><h2>説明</h2><p>この画面は、ゲーム全体の説明書です。迷ったら右側の「説明」を開いてください。下の操作パネルには説明ボタンを置かず、星の操作・保存・ターン終了だけにしています。</p></section><div class="codex-list">
      ...
      <details open><summary>勝利条件</summary><ul><li><b>覇権勝利</b>：Turn50以降、各CPU星系に橋頭堡3拠点・包囲100・首都継承令・降下軍団/要塞攻略教範・恒星チャージ2を満たして母星を攻略します。</li><li><b>支配勝利</b>：有効支配率60%以上、平均開発Lv、行政技術、食料/電力/影響の赤字なしを満たします。Lv0植民地は満額カウントされません。</li><li><b>技術勝利</b>：技術勝利用の最終技術（銀河統治網・特異点中枢・恒星知性）に加え、研究拠点3つ＋拠点Lv合計10、研究系T4・恒星系T4・工業/防衛/交易/虚空T4を各1つ、電力・結晶収入を満たします。条件達成後、<b>銀河同期プロジェクト</b>を進捗100まで進めて成立します（最終技術の取得だけでは勝てません）。軍事・経済・生態・特殊の最終技術は各ルートの補助で、技術勝利には使えません。</li></ul></details>
      ...
      <details><summary>研究分野と最終技術</summary><p>研究は内政・探索・防衛・艦隊・ワープ・経済などに分かれます。技術勝利用の最終技術（研究系）に到達し、銀河同期プロジェクトを完了すると技術勝利です。その他の最終技術は各ルートを補助します。</p><ul>${finalTechs}</ul></details>
  ...
  function showTechDetail(id){
    ...
    const tutGlow=state?.tutorial?.active&&state.tutorial.step===5&&id==='survey'&&can?' tutorial-glow':'';
    const action=`<button class="primary-btn${tutGlow}" data-action="research" data-arg="${id}" type="button" ${can?'':'disabled'}>${btnLabel}</button>`;

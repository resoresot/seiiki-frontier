(() => {
  ...
  const SHIP_AFFINITY = {
    scout:   {militia:1.35, armor:0.65, shield:1.00, flak:0.75},
    frigate: {militia:1.10, armor:1.20, shield:0.80, flak:1.10},
    destroyer:{militia:0.90,armor:1.45, shield:1.05, flak:0.70},
    carrier: {militia:0.80, armor:0.85, shield:1.30, flak:1.45}
  };

  const PLANET_NAMES_POOL = [
    '外縁','辺境','深層','黎明','黄昏','極光','星霧','彗尾',
    '銀嶺','蒼穹','紅塵','翠環','紫苑','白銀','烈風','静海',
    '暁光','薄明','常闇','星影','双月','虹彩','金剛','琥珀'
  ];
  ...
  function createGame(seed, doctrine){
    ...
    return {version:VERSION, seed, turn:1, ap:3, maxAp:3, selectedId:2, systems, factions, logs:[], tutorial:{active:true,step:0}, replay:[], victory:null, sheetCollapsed:false, milestones:{}};
  }
  ...
  function buildCluster(systems, factions, c){
    ...
    for(let i=0;i<planetCount-2;i++){
      ...
      const pname = PLANET_NAMES_POOL[i % PLANET_NAMES_POOL.length];
      const id = addBody(body, body==='belt'?'belt':'planet', `${prefix}${pname}${body==='belt'?'帯':'惑星'}`, dx, dy, {owner:(!isPlayer && i<2 ? fid : null), explored:(!isPlayer && i<3), level:(!isPlayer && i<2 ? 1 : 0), pop:(!isPlayer && i<2 ? 1.6 : 0), parent:star, locked:isPlayer, r:body==='belt'?.038:.046});
      planetIds.push(id);
  ...
  function canvasTouchEnd(e){
    ...
    if(e.touches.length===0){
      if(drag.active && !drag.moved && e.changedTouches && e.changedTouches[0]){
        ...
        if(s){
          const now=Date.now();
          if(lastTapTarget===s.id && now-lastTapTime<400){ showPlanetDetail(s); lastTapTarget=null; }
          else { state.selectedId=s.id; advanceTutorialOnSelect(s); save(); render(); lastTapTarget=s.id; lastTapTime=now; }
        }
      }
  ...
  function canvasPointerUp(e){
    ...
    if(tap){ const s=nearestSystemAt(x,y); if(s){ const now=Date.now(); if(lastTapTarget===s.id && now-lastTapTime<400){ showPlanetDetail(s); lastTapTarget=null; } else { state.selectedId=s.id; advanceTutorialOnSelect(s); save(); render(); lastTapTarget=s.id; lastTapTime=now; } } }
  ...
  function render(){ if(!state) return; renderTop(); renderCoach(); renderPanel(); renderTutorialHighlights(); renderReplay(); draw(); checkVictory(); save(); }
  function renderTop(){
    ...
    RESOURCE_ORDER.forEach(k=>{ const r=RESOURCES[k]; const btn=document.createElement('button'); btn.type='button'; btn.className='resource-chip'; btn.innerHTML=`<img src="${icon(r.icon)}" alt=""><b>${fmt(p.resources[k])}</b><em>${signFmt(income[k]||0)}</em>`; btn.onclick=()=>showResourceHelp(k); bar.appendChild(btn); });
  }
  ...
  function getActionsFor(s){
    ...
    if(canInvade(s)){ const bd=computeBattleDetail(s); a.push({id:'invade',label:'侵攻',warn:true,short:`勝率${bd.winPct}% ${bd.affinityLabel}`}); }
    return a;
  }
  ...
  function canInvade(s){ return !!(s && s.owner && s.owner!=='P' && !faction(s.owner)?.eliminated && hasTech('warpDrive') && player().charge>0 && state.ap>0); }
...
function handleAction(action,arg){ if(!tutorialAllows(action,arg)){ toast('チュートリアル中は、表示された手順だけ実行できます。'); return; } const before=state.tutorial.step; const ok = ({explore,colonize,develop,fortify,chargeStar,invade,buildShip,upgrade,research}[action] || (()=>false))(arg); if(ok) afterTutorialAction(action,arg); render(); }
  ...
  function develop(){ const s=selectedSystem(); if(!s || s.owner!=='P' || s.kind==='star') return toast('通常開発できるのは自領の惑星・衛星・小惑星です。'); if(s.kind==='moon'){ const parent=system(s.parent); if(parent && parent.body!=='home' && !(parent.owner==='P' && parent.level>=1)) return toast('衛星開発には、親惑星の保有と開発Lv1以上が必要です。'); } const cost=developCost(s); if(!canPay(cost)) return toast(`資源不足：${costText(cost)}`); if(!consumeAp()) return false; pay(cost); s.level++; s.pop += hasTech('bio') ? .9 : .55; s.defense += hasTech('defenseGrid') ? 5 : 3; milestone('develop', s); addLog(`${s.name}を開発。収支と防衛が改善。`, 'good'); return true; }
  ...
  function computeAffinityMult(pShips, defType){
    let totalPower=0, weightedAff=0;
    for(const [k,n] of Object.entries(pShips)){
      if(!n || !SHIPS[k]) continue;
      const pw = SHIPS[k].power * n;
      totalPower += pw;
      weightedAff += pw * (SHIP_AFFINITY[k]?.[defType] || 1);
    }
    return totalPower > 0 ? weightedAff / totalPower : 1;
  }
  function computeBattleDetail(s){
    const p=player(), enemy=faction(s.owner);
    const ps=fleetStats(p), es=fleetStats(enemy);
    const defBase = s.defense*2.4 + es.shield*.55 + es.power*.20;
    let defMult = 1;
    if(enemy.techs.includes('planetaryGuns')) defMult*=1.18;
    if(enemy.techs.includes('livingShield')) defMult*=1 + Math.min(.35,s.pop*.02);
    const defense = Math.round(defBase * defMult);
    let attack = ps.attack + ps.power*.38;
    if(hasTech('marines')) attack*=1.25;
    if(hasTech('siegeDoctrine')) attack*=1.12;
    const affinityMult = computeAffinityMult(p.ships, s.defenseType);
    attack *= affinityMult;
    const baseAttack = Math.round(attack);
    const ratio = baseAttack / Math.max(1, defense);
    let winPct;
    if(ratio > 1.5) winPct = Math.min(98, 70 + ratio*12);
    else if(ratio > 1.0) winPct = 50 + (ratio-1)*40;
    else if(ratio > 0.6) winPct = 10 + (ratio-0.6)*100;
    else winPct = Math.max(2, ratio*16);
    winPct = Math.round(clamp(winPct, 2, 98));
    let affinityLabel, affinityTone;
    if(affinityMult >= 1.20){ affinityLabel='有利'; affinityTone='good'; }
    else if(affinityMult >= 1.05){ affinityLabel='やや有利'; affinityTone='good'; }
    else if(affinityMult >= 0.95){ affinityLabel='中立'; affinityTone=''; }
    else if(affinityMult >= 0.80){ affinityLabel='やや不利'; affinityTone='bad'; }
    else { affinityLabel='不利'; affinityTone='bad'; }
    return {ps, es, defense, baseAttack, affinityMult, affinityLabel, affinityTone, winPct, defType:s.defenseType, enemy};
  }
  function invade(){
    const s=selectedSystem();
    if(!canInvade(s)) return toast('侵攻にはワープ航法と恒星チャージが必要です。');
    const detail = computeBattleDetail(s);
    const shipList = Object.entries(SHIPS).filter(([k])=>(player().ships[k]||0)>0).map(([k,sh])=>`<span>${sh.name}×${player().ships[k]}</span>`).join('');
    const defName = DEFENSES[s.defenseType]?.name || '防衛';
    const affinityBars = Object.entries(SHIPS).filter(([k])=>(player().ships[k]||0)>0).map(([k,sh])=>{
      const aff = SHIP_AFFINITY[k]?.[s.defenseType] || 1;
      const pct = Math.round(aff*100);
      const cls = aff>=1.15?'bar-good':aff<=0.85?'bar-bad':'bar-neutral';
      return `<div class="aff-row"><span>${sh.name}</span><div class="aff-bar"><div class="aff-fill ${cls}" style="width:${Math.min(100,pct)}%"></div></div><span>${pct}%</span></div>`;
    }).join('');
    const winColor = detail.winPct>=60?'var(--green)':detail.winPct>=40?'var(--gold)':'var(--red)';
    showModal(`<h2>侵攻確認：${s.name}</h2>
      <div class="battle-preview">
        <div class="bp-side bp-atk"><h3>攻撃側（あなた）</h3><div class="bp-ships">${shipList}</div><div class="bp-stat">攻撃力 <b>${detail.baseAttack}</b></div><div class="bp-stat">総合戦力 <b>${detail.ps.power}</b></div></div>
        <div class="bp-vs"><span class="bp-vs-icon">⚔</span><div class="bp-win" style="color:${winColor}">勝率 ${detail.winPct}%</div></div>
        <div class="bp-side bp-def"><h3>防御側（${detail.enemy.name}）</h3><div class="bp-stat">防衛力 <b>${detail.defense}</b></div><div class="bp-stat">${defName}</div><div class="bp-stat">艦隊 <b>${detail.es.power}</b></div></div>
      </div>
      <div class="panel-section" style="margin-top:8px"><h3>相性（${defName}に対して）</h3><div class="aff-grid">${affinityBars}</div><div class="bp-stat" style="margin-top:6px">総合相性: <b class="${detail.affinityTone}">${detail.affinityLabel}</b>（×${detail.affinityMult.toFixed(2)}）</div></div>
      <p style="font-size:11px;color:#aebfd4;margin-top:8px">乱数により結果は変動します。相性が良い艦種を揃えると勝率が上がります。</p>
      <div class="battle-actions"><button id="battleGo" class="primary-btn" type="button">侵攻開始（AP1 充填1）</button><button id="battleCancel" class="ghost-btn" type="button">撤退</button></div>`, true);
    $('battleCancel').onclick = () => { $('infoModal').close(); };
    $('battleGo').onclick = () => { $('infoModal').close(); executeBattle(s); };
    return false;
  }
  function executeBattle(s){
    if(!consumeAp()) return;
    player().charge--;
    const p=player(), enemy=faction(s.owner);
    const detail = computeBattleDetail(s);
    const atkRoll = 0.72 + Math.random()*0.56;
    const defRoll = 0.82 + Math.random()*0.36;
    const crit = Math.random() < 0.12;
    let finalAtk = detail.baseAttack * atkRoll * (crit ? 1.5 : 1);
    let finalDef = detail.defense * defRoll;
    const won = finalAtk > finalDef;
    const margin = Math.abs(finalAtk - finalDef);
    const marginPct = Math.round(margin / Math.max(1,finalDef) * 100);
    let resultText, resultTone, casualties;
    if(won){
      const oldName = enemy.name;
      s.owner='P'; s.explored=true; s.defense=Math.max(7,Math.round(s.defense*.55));
      loseShips(p, .18);
      casualties = '軽微';
      if(s.homeOf && s.homeOf!=='P'){
        eliminateEmpire(enemy);
        resultText=`${oldName}の母星を制圧！帝国敗退、残存拠点を接収。`;
        addLog(resultText, 'good');
      } else {
        resultText=`${oldName}領${s.name}を制圧！`;
        addLog(resultText, 'good');
      }
      resultTone = 'good';
    } else {
      loseShips(p, .36);
      casualties = '甚大';
      resultText = `${s.name}への侵攻は失敗。防衛を突破できず。`;
      addLog(resultText, 'bad');
      resultTone = 'bad';
    }
    const atkRollPct = Math.round(atkRoll*100);
    const defRollPct = Math.round(defRoll*100);
    showModal(`<h2 class="battle-result-title ${resultTone}">${won?'勝利！':'敗北…'}</h2>
      <div class="battle-result">
        <div class="br-anim ${resultTone}"><div class="br-clash">${won?'💥':'🛡'}</div></div>
        <p class="br-text">${resultText}</p>
        <div class="br-detail">
          <div class="br-row"><span>攻撃力</span><b>${detail.baseAttack}</b><span>× 乱数${atkRollPct}%${crit?' ★会心！':''}</span><b class="good">= ${Math.round(finalAtk)}</b></div>
          <div class="br-row"><span>防衛力</span><b>${detail.defense}</b><span>× 乱数${defRollPct}%</span><b class="bad">= ${Math.round(finalDef)}</b></div>
          <div class="br-row"><span>差分</span><b class="${resultTone}">${won?'+':'−'}${Math.round(margin)} (${marginPct}%)</b></div>
          <div class="br-row"><span>相性</span><b>×${detail.affinityMult.toFixed(2)} ${detail.affinityLabel}</b></div>
          <div class="br-row"><span>損耗</span><b>${casualties}</b></div>
        </div>
      </div>`, true);
    render();
  }
  ...
  function cpuTurn(){
    ...
    state.factions.filter(f=>f.id!=='P'&&!f.eliminated).forEach(f=>{
      const inc=totalIncome(f.id); Object.keys(inc).forEach(k=>inc[k]=Math.ceil(inc[k]*1.45 + (k==='research'?6:k==='materials'?8:k==='energy'?6:3)));
      addRes(inc,f.id); growOwned(f.id);
      const turns = state.turn>6 ? 3 : 2;
  ...
  function chooseCpuAction(f){
    ...
    if(state.turn>10 && f.techs.includes('warpDrive') && (f.charge||0)>0 && pTargets.length && fleetStats(f).power>fleetStats(player()).power*.62){
      const target=pTargets.sort((a,b)=>(b.homeOf==='P')-(a.homeOf==='P') || b.defense-a.defense)[0]; f.charge--; return {fid:f.id,sid:target.id,text:`${f.name}が${target.name}へ圧力をかけた。${persona.quote||''}`,type:'threat'};
    }
    const techs=availableTechs(f.id).filter(t=>!f.techs.includes(t.id) && (t.req||[]).every(r=>f.techs.includes(r))).sort((a,b)=>a.cost-b.cost);
  ...
  function showTechDetail(id){
    ...
    const can=!bought && missing.length===0 && p.resources.research>=t.cost && state.ap>0 && tutorialAllows('research',id);
    const btnLabel=bought?'取得済み':missing.length?'前提不足':p.resources.research<t.cost?'研究不足':state.ap<=0?'AP不足':'この研究を実行';
...
function openMobilePage(page){ modalDetail=null; modalPage=page; const title={empire:'艦隊',tech:'研究',factions:'勢力比較',codex:'説明',log:'ログ'}[page]||'情報'; const html=page==='empire'?empireHtml():page==='tech'?techHtml():page==='factions'?factionsHtml():page==='codex'?codexHtml():logHtml(); showModal(`<h2>${title}</h2>${html}`,true); bindActionButtons($('modalBody')); bindTechButtons($('modalBody')); bindFactionButtons($('modalBody')); if(page==='tech') requestAnimationFrame(()=>setupTechMapScroll()); }
  ...
  function showPlanetDetail(s){
    ...
    const curYield = systemYield(s);
    const maxLevelShow = s.kind==='star' ? 5 : 8;
    let tableRows = '';
    ...
    const growthPotential = s.kind==='star' ? '恒星はチャージ拠点として運用します。レベルを上げるとチャージ上限が増加します。' :
    ...
    showModal(`<h2>${s.name}</h2>
      <div class="planet-detail">
        ...
        <div class="pd-section"><h3>特徴</h3><p>${type.note}</p></div>
